...
public class FabricRestApi {
  ...
  public static Connection CreateAnonymousWebConnection(string Url, bool ReuseExistingConnection = false) {

    string displayName = $"Web-Anonymous-{Url}-{DateTime.Now.ToString()}";
    ...
    var connection = CreateConnection(createConnectionRequest, ReuseExistingConnection);

    return connection;
  }

  public static Connection CreateAzureStorageConnectionWithServicePrincipal(string Server, string Path, bool ReuseExistingConnection = false) {

    string displayName = $"ADLS-SPN-{Server}-{Path}-{DateTime.Now.ToString()}";
  ...
  public static Connection CreateSqlConnectionWithServicePrincipal(string Server, string Database, bool ReuseExistingConnection = false) {

    string displayName = $"SQL-SPN-{Server}:{Database}-{DateTime.Now.ToString()}";
  ...
  public static Connection CreateSqlConnectionWithWorkspaceIdentity(string Server, string Database, bool ReuseExistingConnection = false) {
  ...
  public static void DeleteAllTemplateFiles(string WorkspaceName) {
    string targetFolder = AppSettings.LocalWorkspaceExports + (string.IsNullOrEmpty(WorkspaceName) ? "" : WorkspaceName + @"\");
  ...
  public static void WriteFile(string WorkspaceFolder, string ItemFolder, string FilePath, string FileContent, bool ConvertFromBase64 = true) {
    ...
    string folderPath = AppSettings.LocalWorkspaceExports + WorkspaceFolder + @"\" + ItemFolder;
  ...
  public static Connection CreateAzureStorageConnectionWithServicePrincipal2(string Server, string Path, bool ReuseExistingConnection = false) {
    ...
    return CreateConnection(createConnectionRequest, ReuseExistingConnection);
  }
  ...
  public static Connection CreateAzureStorageConnectionWithWorkspaceIdentity(string Server, string Path, bool ReuseExistingConnection = false) {
    ...
    return CreateConnection(createConnectionRequest, false);
